...
function newSlicer(context, opConfig, executionContext, retryData, logger, client) {
            ...
            .then((startDate) => Promise.resolve(getIndexDate(opConfig.end, 'end'))
                .then((endDate) => {
                    ...
                    if (startDate && endDate) {
                        logger.info(`execution: ${executionConfig.ex_id} start and end range times are ${startDate.format(dateFormat)} and ${endDate.format(dateFormat)}`);
                    }
                    return finalDates;
                }));
    }
    ...
    function makeKeyList(data) {
        ...
        const idSlicer = require('../../id_reader/id_slicer')(context, client, executionContext, idConfig, logger, null, data);
